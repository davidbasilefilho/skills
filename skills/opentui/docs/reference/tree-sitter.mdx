...
OpenTUI integrates Tree-sitter for fast, accurate syntax highlighting. register parsers globally or per client.
...
`aliases` maps additional filetype ids to same parser assets.
...
If `infoStringMap` has no match, fence language label is used as filetype id.
...
Use `updateAssets` utility to download parsers and generate imports.
...
Parser and query files are cached in client `dataPath`. Set custom cache directory:
...
`infoStringToFiletype()` is used for fenced markdown code blocks.
 ...
 extend or override mappings at runtime:
