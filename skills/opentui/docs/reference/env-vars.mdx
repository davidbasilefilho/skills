...
OpenTUI reads environment variables at runtime. Bun loads `.env` automatically, so set these in your shell or in `.env` file.
...
- `OTUI_PALETTE_IDLE_TIMEOUT_MS` bounds palette detection when terminal reports OSC support but does not answer follow-up color queries.
- Disable global `console.*` capture with `OTUI_USE_CONSOLE=false`. `consoleMode` only changes overlay surface.
- `OTUI_NO_NATIVE_RENDER` still runs render loop. In `"split-footer"` mode, current stdout flush path can still emit ANSI cursor movement and clear sequences.
- `OTUI_DUMP_CAPTURES` runs from renderer exit handler. direct `renderer.destroy()` call does not trigger it by itself.
