...
OpenTUI uses `RGBA` class for color representation. class stores 8-bit RGBA channel values plus packed metadata internally, but provides methods for working with different color formats.

Every color also carries **color intent** inside packed value: RGB snapshot, indexed ANSI slot, or terminal default. renderer uses this intent to keep palette-relative colors stable when terminal palette changes.
...
## The parseColor utility
 ...
 `parseColor()` function converts various color formats to RGBA:
...
Every `RGBA` value stores intent in packed metadata next to RGBA snapshot. renderer reads this intent to decide which ANSI sequence to emit:

- `rgb`: literal RGB color. renderer emits `38;2;r;g;b` / `48;2;r;g;b`.
- `default`: terminal's default foreground or background. renderer emits `39` / `49` (SGR default) so terminal picks right side of its palette.
- `indexed`: indexed ANSI color. renderer emits `38;5;n` / `48;5;n` so user's theme overrides for that palette slot keep working when palette changes.

Each color still carries its RGBA values so alpha blending and non-ANSI renderers work. intent flows end-to-end through TypeScript, FFI, buffer, and native renderer.
...
```
 ...
 pass optional snapshot color to override RGBA fallback. renderer uses snapshot for early frames, before it detects terminal palette:
...
`RGBA.buffer` is `Uint16Array(4)`. low byte of each component stores `r`, `g`, `b`, and `a`; high bytes together store 32 bits of metadata.
...
This lets native buffers compare current and next frame colors with exact integer equality. It also keeps palette intent attached to color as it crosses TypeScript/native boundary.
...
- `renderer.getPalette(options?)` — returns `TerminalColors` snapshot. Cached by requested size (`16` or `256`).
- `renderer.paletteDetectionStatus` — `"idle" | "detecting" | "cached"`.
- `renderer.clearPaletteCache()` — drop cached palettes (for example, after user changes their theme).
- `renderer.on("palette", handler)` — subscribe to refreshed palette snapshots when detected palette changes.
...
## Alpha blending
 ...
 use transparent cells and alpha blending for layered effects:
...
Blended colors become literal RGB colors internally. This is intentional: once colors are blended, result no longer represents specific ANSI slot or terminal default.
