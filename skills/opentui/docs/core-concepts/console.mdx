...
OpenTUI includes built-in console overlay that captures all `console.log`, `console.info`, `console.warn`, `console.error`, and `console.debug` calls. position console at any edge of terminal. It supports scrolling and focus management.
...
Toggle console overlay in code:
...
Add keyboard shortcut to toggle console:
...
Terminal UI applications capture stdout for rendering. Regular `console.log` calls would interfere with your interface. console overlay solves this problem:

- Captures all console output without disrupting UI
- Displays logs in dedicated overlay area
