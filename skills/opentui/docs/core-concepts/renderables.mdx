---
title: Renderables
description: building blocks of your terminal UI
order: 2
---

# Renderables

Renderables are building blocks of your UI. position, style, and nest them within each other. Each renderable represents visual element and uses Yoga layout engine for flexible positioning and sizing.

## Creating Renderables

Create renderable by instantiating class with render context ( renderer) and options:
...
Renderables form tree structure. Use `add()` and `remove()` to manage children:
...
Navigate tree to find specific renderables:
...
See [Layout](/docs/core-concepts/layout) page for complete details.
...
By default, left-clicking renderable will auto-focus closest focusable ancestor. Disable this globally with
...
Mouse events bubble up through tree. Stop propagation with `event.stopPropagation()`.
...
When `visible` is `false`, Yoga excludes renderable from layout calculation (equivalent to CSS `display: none`).
...
Opacity affects renderable and all its children.
...
For animations, extend Renderable class and override `onUpdate`:
...
Offset renderable from its layout position (useful for scrolling/animation):
...
This moves renderable visually without affecting layout.

## Buffered Rendering

Enable offscreen rendering for complex content and use hooks to draw to buffer:
...
Clean up renderable and remove it from tree:
