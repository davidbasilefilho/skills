...
If your app loads external TS/TSX modules at runtime (for example file-based plugin system), import this once in your entry file before dynamic imports:
...
- `node` - Function returning JSX element
- `rendererOrConfig` - Optional `CliRenderer` instance or `CliRendererConfig`

### `testRender(node, options?)`

Create test renderer for snapshots and interaction tests.
...
Returns current component catalogue that powers JSX tag lookup.

For plugin slots, see [Plugin Slots overview](/docs/plugins/slots) and [Solid slots](/docs/plugins/solid).

## Scrollback writers

In [`split-footer`](/docs/core-concepts/renderer#split-footer) mode with `externalOutputMode: "capture-stdout"`, Solid binding provides helpers that append JSX-rendered output above footer. They wrap [`renderer.writeToScrollback`](/docs/core-concepts/renderer#writing-to-scrollback) so write scrollback content with signals and components.

### `writeSolidToScrollback(renderer, node, options?)`

Render JSX node once and append it as scrollback commit.
...
If need to pass same JSX rendering to multiple `writeToScrollback` calls (or hold onto writer inside your own code), use lower-level factory:
...
| `trailingNewline` | `boolean` | -              | Append a newline after the final row                                     |
 ...
 writer returns `ScrollbackSnapshot` whose teardown disposes inner Solid subtree after snapshot renders. Use core [`ScrollbackSurface`](/docs/core-concepts/renderer#writing-to-scrollback) APIs directly if you need streaming commits that re-render same tree over time.
...
Run side effects when terminal window gains focus.
...
Run side effects when terminal window loses focus.
...
These hooks listen for terminal focus-in/focus-out events when terminal emulator supports them.

### `useTerminalDimensions()`

Get reactive terminal dimensions (returns Solid signal).
...
Render children into different mount point (useful for modals and overlays).
...
Use [Bun.build](https://bun.sh/docs/bundler) with Solid plugin:
...
To compile to standalone executable:
