...
Text input field with cursor, placeholder text, and focus states. Focus input to receive keyboard input.
...
## Focus states
 ...
 input changes appearance when focused:
...
Fires on every keystroke as value changes:
...
Fires when input loses focus (blur) or when you press Enter, but only if value changed since focus:
...
Fires when user presses Enter/Return:
