...
# FrameBuffer
 ...
 low-level rendering surface for custom graphics and complex visual effects. FrameBuffer provides 2D array of cells with methods optimized for performance and memory.
...
Set single cell's content and colors:
...
Set cell with alpha blending for transparency effects:
...
Draw string of text at position:
...
transforms, and `colorMatrix` when want to target specific cells.
...
1. **Batch updates**: Make multiple changes to frame buffer before next render cycle
