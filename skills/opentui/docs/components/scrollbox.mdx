---
title: ScrollBox
description: scrollable container component with customizable scrollbars
order: 7
---

# ScrollBox

 scrollable container that supports horizontal and vertical scrolling, sticky scroll behavior, viewport culling, and customizable scrollbars.
...
Enable sticky scroll to keep content pinned to edge as new content arrives. Useful for log viewers or chat interfaces.
...
When you scroll away from sticky position, sticky behavior pauses until you scroll back to sticky edge.
...
Style scrollbars using nested options:
...
ScrollBox contains several internal components that style individually:
...
Scroll minimum distance needed to show nested child in viewport. method uses DOM-style "nearest" behavior: if child already fits, call is no-op; otherwise it scrolls to reveal nearest edge.

```typescript
scrollbox.scrollChildIntoView("table-row-42")
```

Use it when you focus element offscreen (search results, newly inserted form field, etc.) and want to bring it into view without moving scroll position more than needed.
