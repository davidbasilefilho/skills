...
Horizontal tab-based selection component with descriptions and scroll support. component must be focused to receive keyboard input.
...
When focused, tab select responds to these keys:
...
Emitted when user presses Enter on tab:
...
Emitted when highlighted tab changes:
...
When there're more tabs than fit in width, component automatically handles horizontal scrolling as you navigate with keyboard.
