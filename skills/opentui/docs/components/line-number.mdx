...
Add line number gutter to renderables that provide line info, such as `CodeRenderable` and text editor components.
...
Set single background for line, or split gutter and content colors separately with `LineColorConfig` object:
...
Assign `undefined` to reset to defaults (`#888888` foreground, transparent background).
